...
- 2026-03-19: Expo Go compatibility fix — `react-native-iap` v14+ uses `react-native-nitro-modules` which crashes on Expo Go load. Added metro-level mock (`src/mocks/react-native-iap.ts`) activated via `EXPO_PUBLIC_MOCK_IAP=true`; updated `metro.config.js` with conditional `resolveRequest` resolver; updated `npm start` / `start:clear` / `start:local` / `start:local:clear` scripts to set the env var automatically. IAP is silently no-op in Expo Go; `android`/`ios` scripts are unaffected. Files changed: `metro.config.js`, `package.json`, `src/mocks/react-native-iap.ts` (new).
